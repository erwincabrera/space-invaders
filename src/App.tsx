...
const KEY_LIST = Object.values(Constants.KEYS).reduce(
  (acc, curr) => Object.values(acc).concat(Object.values(curr)),
  [],
);
...
const App = () => {
  ...
  useEffect(() => {
    const update = () => {
      ...
      if (isKeyDown[Constants.KEYS.MOVEMENT.RIGHT])
        dispatch(Actions.moveRight());
      if (isKeyDown[Constants.KEYS.WEAPONS.PHOTON_TORPEDOS])
        dispatch(Actions.fire());
    ...
    const tickId = setInterval(
      () => ReactDOM.unstable_batchedUpdates(update),
      Constants.TICK_MS,
    );
  ...
  const gameView = (): JSX.Element => <GameView {...state}></GameView>;
  ...
  const loginView = (): JSX.Element => <LoginView onLogin={handleLogin} />;
  ...
  return (
    <div>
      {Object.keys(audioMap).map((name) => (
        <Sound key={name} ref={audioRefs[name]} audio={audioMap[name]} />
      ))}
      {getView()}
    </div>
  );
